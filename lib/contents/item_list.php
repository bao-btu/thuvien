...
if (($ajaxsec_user == $sysconf['ajaxsec_user']) AND ($ajaxsec_passwd == $sysconf['ajaxsec_passwd'])) {
    ...
    if (isset($_POST['id'])) {
        ...
        if ($copy_q->num_rows < 1) {
            ...
            echo '<table width="100%" class="itemList" cellpadding="3" cellspacing="0" style="clear:both">';
				echo '<tr style="border-bottom:1px solid #000">';
				echo '<td width="15%"><strong>'. __('Item Code') .'</strong></td><td width="15%"><strong>'. __('Call Number') .'</strong></td><td width="30%"><strong>'. __('Location') .'</strong></td><td width="40%"><strong>'. __('Status') .'</strong></td>';
            while ($copy_d = $copy_q->fetch_assoc()) {
                ...
                echo '<tr style="border-bottom:1px solid #000">';
                echo '<td width="15%"><strong>'.$copy_d['item_code'].'</strong></td>';
                echo '<td width="15%">'.$copy_d['call_number'].'</td>';
                echo '<td width="30%">'.$copy_d['location_name'];
                if (trim($copy_d['site']) != "") {
                    echo ' ('.$copy_d['site'].')';
                }
                echo '</td>';
                ...
                */
                if ($loan_stat_q->num_rows > 0) {
                    ...
                    echo '<strong width="50%" style="color: red;">'.$copy_d['item_status_name'].'</strong>';
                } else {

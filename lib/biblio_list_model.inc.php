  ...
  protected function makeOutput() {
    ...
    while ($_biblio_d = $this->resultset->fetch_assoc()) {        
      ...
      $_buffer .= '<div class="item"><div class="cover-list">'.$_image_cover.'</div>';
						$_buffer .= '<div class="detail-list">'.$_biblio_d['title'].'/';
      // concat author data
      $_authors = isset($_biblio_d['author'])?$_biblio_d['author']:self::getAuthors($this->obj_db, $_biblio_d['biblio_id']);
      $_edition = isset($_biblio_d['edition'])?$_biblio_d['edition'] : "" ;
      $_publisher = isset($_biblio_d['publisher'])?$_biblio_d['publisher'] : "" ;
      $_publish_year = isset($_biblio_d['publish_year'])?$_biblio_d['publish_year'] : "" ;
      $_collation = isset($_biblio_d['collation'])?$_biblio_d['collation'] : "" ;           
      ...
      $_buffer .= '<div class="subItem">'.$_biblio_d['detail_button'].' '.$_check_mark.'</div>';
    ...
    if (($this->num_rows > $this->num2show)) {
      ...
      $_paging = '';
		...
		if ($_is_member_logged_in) {
			$_submit = '<div style="clear:both; margin-top : 20px"><input class="btn btn-sm btn-primary" type="submit" name="markBiblio" value="'.__('Put marked selection into basket').'" /></div>';

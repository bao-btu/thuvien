        ...
        <div class="divRow">
            ...
            <select name="loanLocation">
                ...
                while($data = $location_query->fetch_assoc()){
                    echo '<option value="' .$data["localtion_id"] . '">' . $data["location_name"] . '</option>';                          
                }
                ?>                
            </select>
            </div>
...
<?php    
    ...
    $sql_report_total = 'SELECT COUNT(loan_id) FROM loan WHERE 1=1';
    $sql_report_member_loan = 'SELECT DISTINCT member_id FROM loan WHERE 1=1';
    $sql_report_by_class = 'SELECT count(DISTINCT  l.member_id) as count, m.inst_name as name FROM loan as l, member as m WHERE l.member_id = m.member_id ';

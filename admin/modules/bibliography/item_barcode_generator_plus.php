...
if (isset($_GET['action']) AND $_GET['action'] == 'print') {
  ...
  $html_str .= 'body {padding: 0; font-family: '.$sysconf['print']['barcode']['barcode_fonts'].'; font-size: '.$sysconf['print']['barcode']['barcode_font_size'].'pt; background: #fff; }'."\n";
  $html_str .= '.labelStyle { width: 35mm; height: 10mm; text-align: center; border: 0px solid #000000; float:left;}'."\n";
  $html_str .= '.width12 {width:12mm;}'."\n";
  $html_str .= '.width15 {width:15mm;}'."\n";  
  $html_str .= '.height7 {clear:both; height:7mm}'."\n";
  $html_str .= '.height8 {clear:both; height:8mm}'."\n"; 
  $html_str .= '.height9 {clear:both; height:9mm}'."\n";
  $html_str .= 'img {opacity: 1;}'."\n";          
  $html_str .= '.labelHeaderStyle { background-color: #CCCCCC; font-weight: bold; padding: 5px; margin-bottom: 5px; }'."\n";
  $html_str .= '</style>'."\n";
  $html_str .= '</head>'."\n";
  $html_str .= '<body style="margin:0; padding:0">'."\n";  
  $html_str .= '<div style="width: 100%; border : 0px solid black">';
  $thu_tu_dong = 1;
  foreach ($chunked_barcode_arrays as $barcode_rows) {          // Toan bo barcode
      $thu_tu_item = 1;      
      
      $html_str .= '<div style="clear:both">';      
      foreach ($barcode_rows as $barcode) {                     // 3 barcode 1 dong
            $html_str .= '<div class="labelStyle">';
            $html_str .= '<img src="'.SWB.IMG.'/barcodes/'.str_replace(array(' '), '_', $barcode[1]).'.png" style="height:10mm; width:35mm" border="0" />';            
            $html_str .= '</div>';
            if($thu_tu_item == 1)
            {
                $html_str .= '<div class="labelStyle width15">&nbsp;</div>';
                $thu_tu_item = 2;                    
            }else if($thu_tu_item == 2)
            {
                $html_str .= '<div class="labelStyle width12">&nbsp;</div>';
                $thu_tu_item = 3;
            }else
            {
      ...
      $html_str .= '</div>';
      if($thu_tu_dong == 1 || $thu_tu_dong == 2 || $thu_tu_dong == 5 || $thu_tu_dong == 9)
      {
        $html_str .= '<div class="height7 dong'.$thu_tu_dong.'">&nbsp;</div>';
        $thu_tu_dong++;
      }else if ($thu_tu_dong == 4 || $thu_tu_dong == 6 || $thu_tu_dong == 3 )
      {
        $html_str .= '<div class="height9 dong'.$thu_tu_dong.'">&nbsp;</div>';
        $thu_tu_dong++;
      }else if ($thu_tu_dong == 7 || $thu_tu_dong == 8)
      {
        $html_str .= '<div class="height8 dong'.$thu_tu_dong.'">&nbsp;</div>';
        $thu_tu_dong++;
      }else if ($thu_tu_dong == 10)
      {
        $html_str .= '<div style="clear:both; height: 22mm;">&nbsp;</div>';
        $thu_tu_dong = 1;
      }
  }
  $html_str .= '</div>';
  
  //BAO $html_str .= '<script type="text/javascript">self.print();</script>'."\n";
  $html_str .= '</body></html>'."\n";
  // unset the session
  unset($_SESSION['barcodes']);
  // write to file
...
if (isset($_GET['keywords']) AND $_GET['keywords']) {
  ...
  $biblio_list = new biblio_list($dbs, 60);
...
$datagrid_result = $datagrid->createDataGrid($dbs, $table_spec, 60, $can_read);
if (isset($_GET['keywords']) AND $_GET['keywords']) {

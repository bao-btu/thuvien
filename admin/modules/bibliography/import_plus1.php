      ...
      if ($row_count < $record_offset) {
        ...
        continue;
          ...
          if ($field) {
              ...
              $title = '\''.$title.'\'';
              $gmd_id = utility::getID($dbs, 'mst_gmd', 'gmd_id', 'gmd_code', $field[1], $gmd_id_cache);
              ...
              $coll_type = (integer)utility::getID($dbs, 'mst_coll_type', 'coll_type_id', 'coll_type_name', $field[25], $ct_id_cache);
			  
              $authors = trim($field[15]);
              $subjects = trim($field[16]);
              
              $itemsKhoMuon = trim($field[17]);
              $soLuongKhoMuon = trim($field[18]);
              $itemsKhoDoc = trim($field[19]);
              $soLuongKhoDoc = trim($field[20]);
              $itemsKhoCS3 = trim($field[21]);
              $soLuongKhoCS3 = trim($field[22]);
              $price = $field[23]?'\''.$field[23].'\'':'NULL';
              $price_currency = $field[24]?'\''.$field[24].'\'':'NULL';
              $nhomSach = $field[25]?'\''.$field[25].'\'':'NULL';;
              $maNhomSach = trim($field[26]);
              ...
              if (!$dbs->error) {
                  ...
                  if (!empty($subjects)) {
					...
					// items Kho muon
					if (!empty($itemsKhoMuon) && !empty($soLuongKhoMuon)) {
						$location = utility::getID($dbs, 'mst_location', 'location_id', 'location_id', 'PM', $loc_id_cache);
						$location = $location?'\''.$location.'\'':'NULL';
						//$location = 'PM';
						$item_status = utility::getID($dbs, 'mst_item_status', 'item_status_id', 'item_status_name', '', $stat_id_cache);
						$item_status = $item_status?'\''.$item_status.'\'':'NULL';                        
                       
						$item_sql = 'INSERT IGNORE INTO item (item_code, biblio_id, call_number, coll_type_id, location_id, item_status_id, source, price, price_currency, input_date, last_update) VALUES ';
						
						$soTT = (int)$itemsKhoMuon;
						$soLuongKhoMuon = (int)$soLuongKhoMuon;												
						for($i = 0; $i < $soLuongKhoMuon ; $i++) {														
							$maSach = "M." . $soTT . "." . $maNhomSach;
							$item_sql .= " ( '$maSach', $biblio_id, $call_number, $coll_type, $location, $item_status, 1, $price, $price_currency, $curr_datetime, $curr_datetime),";							
							$soTT++;
						}
						// remove last comma
					...
					// items Kho doc
					if (!empty($itemsKhoDoc) && !empty($soLuongKhoDoc)) {
						$location = utility::getID($dbs, 'mst_location', 'location_id', 'location_id', 'PD', $loc_id_cache);
						$location = $location?'\''.$location.'\'':'NULL';
						//$location = 'PD';
						$item_status = utility::getID($dbs, 'mst_item_status', 'item_status_id', 'item_status_name', '', $stat_id_cache);
						$item_status = $item_status?'\''.$item_status.'\'':'NULL';                        
						...
						$soTT = (int)$itemsKhoDoc;
						$soLuongKhoDoc = (int)$soLuongKhoDoc;												
						for($i = 0; $i < $soLuongKhoDoc ; $i++) {														
							$maSach = "D." . $soTT . "." . $maNhomSach;
							$item_sql .= " ( '$maSach', $biblio_id, $call_number, $coll_type, $location, $item_status, 1, $price, $price_currency, $curr_datetime, $curr_datetime),";							
							$soTT++;
						}
						// remove last comma
					...
					// items Kho CS3
					if (!empty($itemsKhoCS3) && !empty($soLuongKhoCS3)) {
						$location = utility::getID($dbs, 'mst_location', 'location_id', 'location_id', 'CS3', $loc_id_cache);
						$location = $location?'\''.$location.'\'':'NULL';
						//$location = 'CS3';
						$item_status = utility::getID($dbs, 'mst_item_status', 'item_status_id', 'item_status_name', '', $stat_id_cache);
						$item_status = $item_status?'\''.$item_status.'\'':'NULL';                        
						...
						$soTT = (int)$itemsKhoCS3;
						$soLuongKhoCS3 = (int)$soLuongKhoCS3;												
						for($i = 0; $i < $soLuongKhoCS3 ; $i++) {														
							$maSach = "C." . $soTT . "." . $maNhomSach;
							$item_sql .= " ( '$maSach', $biblio_id, $call_number, $coll_type, $location, $item_status, 1, $price, $price_currency, $curr_datetime, $curr_datetime),";							
							$soTT++;
						}
						// remove last comma

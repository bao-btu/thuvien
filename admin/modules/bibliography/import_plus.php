      ...
      if ($row_count < $record_offset) {
        ...
        continue;
          ...
          if ($field) {
              ...
              $itemsKhoDoc = trim($field[18]);
              $itemsKhoCS3 = trim($field[19]);
              $price = $field[20]?'\''.$field[20].'\'':'NULL';
              $price_currency = $field[21]?'\''.$field[21].'\'':'NULL';
              ...
              if (!$dbs->error) {
                  ...
                  $coll_type = (integer)utility::getID($dbs, 'mst_coll_type', 'coll_type_id', 'coll_type_name', 'Sách tham khảo', $ct_id_cache);                  
                  // items Kho muon
                  if (!empty($itemsKhoMuon)) {
                        $location = utility::getID($dbs, 'mst_location', 'location_id', 'location_name', 'Phòng mượn', $loc_id_cache);
                        ...
                        /*                                              
                        $sql_str = "INSERT INTO item (item_code, coll_type_id, location_id, item_status_id, source, price, price_currency)
                            VALUES ($item_code, $coll_type, $location, $item_status, $source, $price, $price_currency)";
                        */    
                      $item_sql = 'INSERT IGNORE INTO item (item_code, biblio_id, call_number, coll_type_id, location_id, item_status_id, source, price, price_currency) VALUES ';
                      $item_array = explode('><', $itemsKhoMuon);
                      foreach ($item_array as $item) {
                          $item = trim(str_replace(array('>', '<'), '', $item));
                          $item_sql .= " ('$item', $biblio_id, $call_number, $coll_type, $location, $item_status, 1, $price, $price_currency),";
                      }
                      // remove last comma
                  ...
                  // items Kho doc
                  if (!empty($itemsKhoDoc)) {
                        $location = utility::getID($dbs, 'mst_location', 'location_id', 'location_name', 'Phòng đọc', $loc_id_cache);
                        $location = $location?'\''.$location.'\'':'NULL';
                        $item_status = utility::getID($dbs, 'mst_item_status', 'item_status_id', 'item_status_name', 'Mượn đọc', $stat_id_cache);
                        $item_status = $item_status?'\''.$item_status.'\'':'NULL';                        
                        /*                                              
                        $sql_str = "INSERT INTO item (item_code, coll_type_id, location_id, item_status_id, source, price, price_currency)
                            VALUES ($item_code, $coll_type, $location, $item_status, $source, $price, $price_currency)";
                        */    
                      $item_sql = 'INSERT IGNORE INTO item (item_code, biblio_id, call_number, coll_type_id, location_id, item_status_id, source, price, price_currency) VALUES ';
                      $item_array = explode('><', $itemsKhoDoc);
                      foreach ($item_array as $item) {
                          $item = trim(str_replace(array('>', '<'), '', $item));
                          $item_sql .= " ('$item', $biblio_id, $call_number, $coll_type, $location, $item_status, 1, $price, $price_currency),";
                      }
                  ...
                  // items Kho CS3
                  if (!empty($itemsKhoCS3)) {
                        $location = utility::getID($dbs, 'mst_location', 'location_id', 'location_name', 'Cơ sở 3', $loc_id_cache);
                        $location = $location?'\''.$location.'\'':'NULL';
                        $item_status = utility::getID($dbs, 'mst_item_status', 'item_status_id', 'item_status_name', '', $stat_id_cache);
                        $item_status = $item_status?'\''.$item_status.'\'':'NULL';                        
                        /*                                              
                        ...
                        */    
                      $item_sql = 'INSERT IGNORE INTO item (item_code, biblio_id, call_number, coll_type_id, location_id, item_status_id, source, price, price_currency) VALUES ';
                      $item_array = explode('><', $itemsKhoCS3);
                      foreach ($item_array as $item) {
                          $item = trim(str_replace(array('>', '<'), '', $item));
                          $item_sql .= " ('$item', $biblio_id, $call_number, $coll_type, $location, $item_status, 1, $price, $price_currency),";
                      }
